    ...
    <div class="col-xs-12 col-sm-6 col-lg-3 margin-bottom-10">
        <div class="col-xs-12 padding-10 border shadow">
            <div class="col-xs-5 padding-0">
                <div class="btn btn-primary btn-sm add-creation col-xs-12" data-id-creation="{$product.id}"><span class="glyphicon glyphicon-shopping-cart"></span> <span class="hidden-xs">Add to cart</span></div>
            </div>
            ...
            <div class="popup-share-{$product.id} hidden">
                <a target="_blank" class="btn btn-default col-xs-6 margin-auto color-facebook bold social-network-share" href="https://www.facebook.com/sharer/sharer.php?u={$base_dir_ssl}shop/{$shop.name}?id_product={$product.id_product}%26id_creation={$product.id}%26id_design={$product.id_design}" >
                    <span class="fa fa-facebook"></span> Share on Facebook
                </a>
                <a target="_blank" class="btn btn-default col-xs-6 margin-auto color-twitter margin-top-10 bold social-network-share" href="https://twitter.com/home?status={$base_dir_ssl}shop/{$shop.name}?id_product={$product.id_product}%26id_creation={$product.id}%26id_design={$product.id_design}">
                    <span class="fa fa-twitter"></span> Share on Twitter
                </a>
                <a target="_blank" class="btn btn-default col-xs-6 margin-auto color-googleplus margin-top-10 bold social-network-share" href="https://plus.google.com/share?url={$base_dir_ssl}shop/{$shop.name}?id_product={$product.id_product}%26id_creation={$product.id}%26id_design={$product.id_design}">
                    <span class="fa fa-google-plus"></span> Share on Google+
                </a>
                <a target="_blank" class="btn btn-default col-xs-6 margin-auto color-linkedin margin-top-10 bold social-network-share" href="https://www.linkedin.com/shareArticle?mini=true&title={$product.product_name|escape:'html':'UTF-8'}&summary=&source=&url={$base_dir_ssl}shop/{$shop.name}?id_product={$product.id_product}%26id_creation={$product.id}%26id_design={$product.id_design}">
                    <span class="fa fa-linkedin"></span> Share on Linkedin
                </a>
                <a target="_blank" class="btn btn-default col-xs-6 margin-auto color-pinterest margin-top-10 bold social-network-share" href="https://www.pinterest.com/pin/create/button/" data-pin-url="{$base_dir_ssl}shop/{$shop.name}?id_product={$product.id_product}%26id_creation={$product.id}%26id_design={$product.id_design}" data-pin-media="{$base_dir_ssl}img/custom_shop/creation/{$product.custom_img}" data-pin-custom="true">
