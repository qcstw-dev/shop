...
<form action="{$base_uri}shop/{$custom_shop_name}/checkout?step=4" method="post">
{else}
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-6">
                <div id="address_invoice_form" class="form-group selector1"{if $cart->id_address_invoice == $cart->id_address_delivery} style="display: none;"{/if}>
                    ...
                    {else}
                        <a href="{$link->getPageLink('custom-shop-address', true, NULL, "shop_name={$custom_shop_name}&back={$base_uri}shop/{$custom_shop_name}/checkout?step=3")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn-sm btn btn-default icon-right">
...
<script>
    ...
    {/capture}
    {capture name=addressUrl}{$link->getPageLink('custom-shop-address', true, NULL, 'back='|cat:$base_uri|cat:'shop/'|cat:{$custom_shop_name}|cat:'/checkout'|cat:'?step=3')|escape:'quotes':'UTF-8'}{/capture}
    ...
    {capture name=addressUrlAdd}{$smarty.capture.addressUrl|cat:'&shop_name='|cat:$custom_shop_name|cat:'&id_address='}{/capture}
