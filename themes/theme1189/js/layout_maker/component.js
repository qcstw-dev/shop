...
var resizeableImage = function (image_target, customizable) {
    ...
    rotate = function (value, width, height, type) {
        ...
        if (type === 'product') {
            new_image_resized = image_overlay;
        } else {
            rotate_value = value;
        ...
        context.rotate(TO_RADIANS * value);
        if ($.inArray(value, [0, 180, 360]) !== -1) {
        ...
        if (type === 'product') {
            $(image_overlay).attr('src', resize_canvas.toDataURL("image/png"));
        } else {
...
//        ctx.drawImage(image_target, 0, 0, width, height, $('.resize-container').offset().left - $('.component').offset().left, $('.resize-container').offset().top - $('.component').offset().top, width, height);
//        ctx.drawImage(image_overlay, left_overlay, top_overlay, width, height);
        drawImageIOSFix(ctx, image_target, 0, 0, width, height, $('.resize-container').offset().left - $('.component').offset().left, $('.resize-container').offset().top - $('.component').offset().top, width, height);
        drawImageIOSFix(ctx, image_overlay, left_overlay, top_overlay, 600, 600, 0, 0, width, height);
...
$('.rotate').live('click', function () {
    if ($.inArray(rotate_value, [0, 90, 180, 270]) !== -1) {
        rotate(rotate_value + 90, $('.resize-container').width(), $('.resize-container').height());
    } else if (rotate_value === 360) {
        rotate(90, $('.resize-container').width(), $('.resize-container').height());
    } else {
        rotate(90, $('.resize-container').height(), $('.resize-container').width());
    }
});
$('.rotate-product').live('click', function () {
    rotate(90, $('.overlay-img').width(), $('.overlay-img').height(), 'product');
});
/**
