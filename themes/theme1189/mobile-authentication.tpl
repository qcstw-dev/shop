...
{if !isset($email_create)}
    <div class="row">
        <div class="col-xs-12 col-sm-{12/$linkWidth}">
            <form action="{$link->getPageLink('mobile-authentication', true)|escape:'html':'UTF-8'}" method="post" id="create-account_form" class="box">
                <div class="font-size-15 bold">{l s='Create an account'}</div>
                <hr>
        ...
        <div class="col-xs-12 col-sm-{12/$linkWidth}">
            <form action="{$link->getPageLink('mobile-authentication', true)|escape:'html':'UTF-8'}" method="post" id="login_form" class="box">
                <div class="font-size-15 bold">{l s='Already registered?'}</div>
                <hr>
                <div class="form_content clearfix">
                    <div class="form-group">
    ...
    {if false && isset($inOrderProcess) && $inOrderProcess && $PS_GUEST_CHECKOUT_ENABLED}
        <form action="{$link->getPageLink('mobile-authentication', true, NULL, "back=$back")|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="std clearfix">
            <div class="box">
                <div id="opc_account_form" style="display: block; ">
                    <div class="font-size-15 bold">{l s='Instant checkout'}</div>
                    <!-- Account -->
                    ...
                    {/if}
                    <h3 class="page-heading bottom-indent top-indent">{l s='Delivery address'}</div>
                ...
                <div id="opc_invoice_address"  class="unvisible">
                    ...
                    {assign var=postCodeExist value=false}
                    {assign var=dniExist value=false}
                    <h3 class="page-subheading top-indent">{l s='Invoice address'}</div>
...
{/if}-->
<form action="{$link->getPageLink('mobile-authentication', true)|escape:'html':'UTF-8'}" method="post" id="account-creation_form" class="std box">
    ...
    {if $b2b_enable}
        <div class="account_creation">
            <div class="font-size-15 bold">{l s='Your company information'}</div>
            <p class="form-group">
    ...
    {if isset($PS_REGISTRATION_PROCESS_TYPE) && $PS_REGISTRATION_PROCESS_TYPE}
        <div class="account_creation">
            <div class="font-size-15 bold">{l s='Your address'}</div>
            {foreach from=$dlv_all_fields item=field_name}
                {if $field_name eq "company"}
                    {if !$b2b_enable}
                        <p class="form-group">
        ...
        <div class="account_creation dni">
            <div class="font-size-15 bold">{l s='Tax identification'}</div>
            <p class="required form-group">
                <label for="dni">{l s='Identification number'} <sup>*</sup></label>
    ...
    <script>
        {strip}
            ...
            {if isset($smarty.post.id_state_invoice) && isset($smarty.post.id_state_invoice) && $smarty.post.id_state_invoice}
                var idSelectedStateInvoice={$smarty.post.id_state_invoice|intval};
            {else}
            ...
            {if isset($smarty.post.id_country_invoice) && isset($smarty.post.id_country_invoice) && $smarty.post.id_country_invoice}
                var idSelectedCountryInvoice={$smarty.post.id_country_invoice|intval};
            {else}
            ...
            {if isset($email_create) && $email_create}
                var email_create={$email_create|boolval};
            {else}
