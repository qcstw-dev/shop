...
class CustomShopFrontCheckoutControllerCore extends CustomShopFrontControllerCore {
            ...
            if (!($code = trim(Tools::getValue('discount_name')))) {
                ...
                if (($cartRule = new CartRule(CartRule::getIdByCode($code))) && Validate::isLoadedObject($cartRule)) {
                    if ($error = $cartRule->checkValidity($this->context, false, true)) {
                        ...
                        header('Location: ' . _PS_BASE_URL_ . __PS_BASE_URI__ . 'shop/' .$this->custom_shop['name'].'/checkout');
                    }
                } else {
    ...
    public function initContent() {
        ...
        if ($cart_products || Tools::getValue('step') == 'order-confirmation') {
            ...
            if ($isLogged && Tools::getValue('step') == 2) {
                header('Location: ' . _PS_BASE_URL_ . __PS_BASE_URI__ .'shop/' .$this->custom_shop['name'].'/checkout?step=3');
            }
            if (!$isLogged && in_array(Tools::getValue('step'), ['3', '4', '5'])) {
                header('Location: ' . _PS_BASE_URL_ . __PS_BASE_URI__ .'shop/' .$this->custom_shop['name'].'/checkout?step=2');
            }
            ...
            switch (Tools::getValue('step')) {
                case 1: // Summary
                    $this->context->smarty->assign('custom_shop_summary', _PS_THEME_DIR_ . 'custom-shop-summary.tpl');
                    break;
                case 2: // Signin
                    $this->context->smarty->assign('custom_shop_login', _PS_THEME_DIR_ . 'custom-shop-login.tpl');
                    break;
                case 3: // Address
                    ...
                    $this->context->smarty->assign('back', _PS_BASE_URL_ . __PS_BASE_URI__ .'shop/' .$this->custom_shop['name'].'/checkout?step=3');
                    $this->context->smarty->assign('opc', false);
                    $this->context->smarty->assign('custom_shop_addresses', _PS_THEME_DIR_ . 'custom-shop-order-address.tpl');
                    break;
                case 4: // Shipping
                    ...
                    $this->context->smarty->assign('custom_shop_shipping', _PS_THEME_DIR_ . 'custom-shop-order-carrier.tpl');
                    break;
                case 5:
                    if (!$this->context->cart->id_address_delivery) {
                        header('Location: ' . _PS_BASE_URL_ . __PS_BASE_URI__ .'shop/' .$this->custom_shop['name'].'/checkout?step=3');
                    ...
                    $this->context->smarty->assign('custom_shop_payment', _PS_THEME_DIR_ . 'custom-shop-payment.tpl');
                    break;
                case 'order-confirmation':
                    ...
                    if ($order_state->template[$this->context->language->id] == 'payment_error') {
                        ...
                        $this->context->smarty->assign('custom_shop_payment_confirmation', _PS_THEME_DIR_ . 'custom-shop-error-payment.tpl');
                    } else {
                        ...
                        $this->context->smarty->assign('custom_shop_payment_confirmation', _PS_THEME_DIR_ . 'custom-shop-order-confirmation.tpl');
                    }
                    break;
                default:
                    $this->context->smarty->assign('custom_shop_summary', _PS_THEME_DIR_ . 'custom-shop-summary.tpl');
            }
            $this->setTemplate(_PS_THEME_DIR_ . 'custom-shop-checkout.tpl');
        } else {
            header('Location: ' . _PS_BASE_URL_ . __PS_BASE_URI__ . 'shop/' .$this->custom_shop['name']);
        }
    }
...
//                            $product['id_product'], !Product::getTaxCalculationMethod(), $product['id_product_attribute'], 6, null, false, false, 1, false, null, null, null, $null, true, true, $cart_product_context, null, $product['custom_picture'], $product['id_customized_prod']);
//
