...
class ProductPopupControllerCore extends FrontController {
    ...
    public function initContent() {
        ...
        if (!$this->errors) {
            ...
            foreach ($aQuantities as $iQuantity) {
                ...
                if (Tools::getValue('side') && Tools::getValue('side') === 'front') {
                    $aPrices[$iQuantity] += CustomShopDesign::getPrice(Tools::getValue('id_design'));
                }
            }
            $this->context->smarty->assign(array(
    ...
    protected function assignImages() {
        ...
        foreach ($images as $k => &$image) {
            if ($image['cover']) {
                if (Tools::getValue('side') && Tools::getValue('id_creation') && Tools::getValue('side') === 'front') {
                    $image['cover'] = __PS_BASE_URI__.'img/custom_shop/creation/'.CustomShopProduct::getProductById(Tools::getValue('id_creation'))['custom_img'];
                }
